...
부팅 시 자동 실행을 켜고, macOS 입력 모니터링 권한 상태를
확인합니다. 권한을 허용하면 앱을 다시 실행하지 않아도 바로 반영돼요.
...
> **macOS**: 전역 키 입력 감지는 **입력 모니터링** 권한이 필요합니다
> (손쉬운 사용 권한과는 다릅니다). 환영 화면이나 설정 창의 **허용**
> 버튼을 누르면 시스템 권한 창이 떠요. **시스템 설정 → 개인정보 보호
> 및 보안 → 입력 모니터링**에서 직접 켤 수도 있습니다. 권한을 허용하면
> 앱을 다시 실행하지 않아도 바로 적용돼요. 권한이 없어도 캐릭터는
> 동작하지만 타이핑에는 반응하지 않습니다.
